(function(provide) {
  ...
  /**
   * Creates an SVG pie chart within the given
   * DOM node.
   *
   * @param domNode
   * @param data
   * @param options
   */
  var PieChart = function( node, data, options ) {

    this.node = node;
    this.options = options || {};
    var radius = $( this.node ).width() / 4;
  ...
  PieChart.prototype = {
    ...
    render : function() {
      ...
      var paths = arcs.append( 'path' )
                    ...
                    .attr( 'd', this.options.arc);
      if( this.options.animate.status ) {
        this.animate();
      }
    },
  ...
  provide('components/graphing', { PieChart : PieChart, BarChart : BarChart });
})(provide);